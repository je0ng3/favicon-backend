...
public class S3Controller {
    ...
    @PostMapping("/upload")
    public ResponseEntity<APIResponse<String>> uploadFile(@RequestParam("files") MultipartFile[] files) throws IOException {
    ...
    public ResponseEntity<APIResponse<String>> deleteFile(@PathVariable Long resourceId) {
        Resource resource = resourceRepository.findById(resourceId)
                .orElseThrow(() -> new ResourceNotFoundException("Resource를 찾을 수 없습니다: " + resourceId));
