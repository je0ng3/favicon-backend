...
import com.capstone.favicon.dataset.dto.TrendResponseDto;
...
public class TrendController {
    ...
    public ResponseEntity<APIResponse<?>> getTrendsByDate(@RequestParam("date") LocalDate date) {
        return ResponseEntity.ok().body(APIResponse.successAPI("success", toDto(trendService.getTrendsByDate(date))));
    }
    ...
    public ResponseEntity<APIResponse<?>> getTrendsByDatasetId(
        ...
        return ResponseEntity.ok()
                .body(APIResponse.successAPI("success", toDto(trendService.getTrendsByDatasetId(datasetId, startDate, endDate))));
    }
    ...
    private List<TrendResponseDto> toDto(List<Trend> trends) {
        return trends.stream().map(TrendResponseDto::from).toList();
    }
}
